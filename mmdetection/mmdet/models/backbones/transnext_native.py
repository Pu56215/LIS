...
class TransNeXt(BaseModule):
    ...
    def __init__(self, img_size=224, pretrain_size=None, window_size=[3, 3, 3, None],
                 ...
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], num_stages=4, pretrained=None, is_extrapolation=False,init_cfg=None):
        super(TransNeXt, self).__init__(init_cfg)
        ...
        for i in range(self.num_stages):
            ...
            with torch.no_grad():
                if i != (self.num_stages - 1):
                    ...
                    seq_length_scale = torch.log(torch.tensor((H // sr_ratio) * (W // sr_ratio),dtype=torch.float, device=x.device))
                    padding_mask = None
            for blk in block:
